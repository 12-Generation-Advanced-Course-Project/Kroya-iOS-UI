...
struct LoginScreenView: View {
        ...
        ZStack {
            NavigationView {
                VStack(spacing: 0) {
                    ...
                    NavigationLink(
                        destination: FillPasswordScreen(),
    ...
    private func destinationView() -> some View {
        if authVM.isOTPSent {
            VerificationCodeView(userStore: userStore) // Show this view if OTP has been sent
        } else if authVM.isEmailExist {
            FillPasswordScreen() // Show this if email exists
        } else {
            EmptyView() // No navigation
        }
    }
}
