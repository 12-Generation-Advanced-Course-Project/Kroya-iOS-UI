...
//                    }
                    ...
                    if Auth.shared.hasAccessToken(){
                        if recipeViewModel.isLoading {
                            ...
                            FoodRecipeView(isChooseCuisine: isChooseCuisine, foodRecipeViewModel:  recipeViewModel)
//                            ScrollView {
//                                LazyVStack {
//                                    ForEach(isChooseCuisine ? recipeViewModel.RecipeByCategory : recipeViewModel.filteredFoodRecipeList) { recipe in
//                                        NavigationLink(destination:
...
struct FoodRecipeView: View {
    ...
    @ObservedObject var foodRecipeViewModel:  RecipeViewModel

    var body: some View {
        if !foodRecipeViewModel.filteredFoodRecipeList.isEmpty {
            ScrollView {
                LazyVStack {
                    ForEach(foodRecipeViewModel.filteredFoodRecipeList) {recipe in
                        NavigationLink(destination:
                                        FoodDetailView(
                                            isFavorite: recipe.isFavorite ?? false,
                                            showPrice: false, // Always false for recipes
